...
UserDir="/home/jasonshi/.config/libreoffice/4/user/TexMaths/"
PkgDir="/home/jasonshi/.config/libreoffice/4/user/uno_packages/cache/uno_packages/lu2909873y5u5x.tmp_/TexMaths-0-v2.49.oxt/"
...
if [ "${ext}" = "svg" ]
then
   "/usr/bin/dvisvgm" -n1 -e ${filename}.dvi > ${filename}.dat 2>&1
fi
fi
...
if [ -f ${filename}.xdv ]
then
"/usr/bin/dvisvgm" -n1 -e ${filename}.xdv > ${filename}.dat 2>&1
fi
fi
